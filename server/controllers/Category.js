...
exports.categoryPageDetails = async (req, res) => {
	try {
		...
		const selectedCategory = await Category.findById(categoryId)          //populate instuctor and rating and reviews from courses
			.populate({path:"courses",match:{status:"Published"},populate:([{path:"instructor"},{path:"ratingAndReviews"},{path:"courseContent"}])})
			.exec();
		console.log("backend output",selectedCategory);
