...
exports.verifySignature = async (req, res) => {
        ...
        const {courses} = req.body;
        const userId = req.user.id;
        console.log("verify courses=",courses)
            ...
            if(!courses || !userId) {
                return res.status(400).json({
                    ...
                    try{
                        ...
                        const emailTemplate = courseEnrollmentEmail(courseName,userName );
                        await mailSender(
                            userEmail,
        ...
        try{
            ...
            console.log("generated secret",generatedSignature)
            if(generatedSignature === razorpay_signature) {
                const data =  await enrolleStudent(courses, userId);
                return res.status(200).json({
                    success:true,
                    data :data
                });
                ...
                return res.status(500).json({
                    success:false,
                    message : 'generated signature not match',
