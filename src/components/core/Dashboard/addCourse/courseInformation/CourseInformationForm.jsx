...
const CourseInformationForm = () => {
    ...
    const onSubmit = async(data) => {
        ...
        if(editCourse) {
            ...
            setLoading(true);
            console.log("printing data at services",formData)
            const result = await editCourseDetails(formData, token.token);
            console.log("printing result for edit",result)
            setLoading(false);
            if(result) {
        ...
        console.log("BEFORE add course API call");
        console.log("PRINTING FORMDATA", formData);
        const result = await addCourseDetails(formData,token.token);
        if(result) {
            dispatch(setStep(2));
            dispatch(setCourse(result));
        }
  ...
  return (
        <>
            <form onSubmit = {handleSubmit(onSubmit)} className='p-5 flex flex-col gap-7'>
